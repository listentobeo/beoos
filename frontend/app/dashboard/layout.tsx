import { DashboardAutoRefresh } from "@/components/dashboard/dashboard-auto-refresh";
import { OperatorAssistant } from "@/components/dashboard/operator-assistant";
import { Sidebar } from "@/components/dashboard/sidebar";
import { beoApi } from "@/lib/api";
import { activeBusiness, type Business, type InboxStats } from "@/lib/api";

export const metadata = {
  robots: { index: false, follow: false },
};

export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  let businesses: Business[] = [];
  let activeId: string | null = null;
  let inboxStats: InboxStats | null = null;
  try {
    businesses = await beoApi.businesses();
    activeId = (await activeBusiness(businesses))?.id ?? null;
    if (activeId) {
      inboxStats = await beoApi.stats(activeId);
    }
  } catch {
    // The page below presents a connection state; the shell should remain usable.
  }
  return (
    <div className="min-h-screen">
      <Sidebar businesses={businesses} activeId={activeId} inboxStats={inboxStats} />
      <DashboardAutoRefresh />
      <OperatorAssistant businessId={activeId} />
      <main className="min-h-screen pt-[150px] transition-[padding] duration-300 ease-out lg:pl-[var(--beoos-sidebar-width,264px)] lg:pt-0">
        {children}
      </main>
    </div>
  );
}
